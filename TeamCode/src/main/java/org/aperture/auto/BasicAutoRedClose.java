...
@Autonomous
public class BasicAutoRedClose extends OpModeCommon {
    ...
    XyhVector preloadDepoRight = new XyhVector(startPos.x-59,  startPos.y+90, Math.toRadians(-97));
    XyhVector preloadDepoCenter = new XyhVector(startPos.x-75,startPos.y+90,Math.toRadians(-101));
    XyhVector preloadDepoLeft = new XyhVector(startPos.x-90,startPos.y+90,Math.toRadians(-90));
    XyhVector preloadLeftPush = new XyhVector(preloadDepoLeft.x,preloadDepoLeft.y-26,Math.toRadians(-90));
    ...
    public void runOpMode() throws InterruptedException {
        ...
        initRedWebcam();
        MarkerPos markerPos = getMarkerPosRed(true);
        ...
        switch (markerPos) {
            ...
            case RIGHT:
                ...
                intakeExtension = INITIAL_INTAKE_EXTENSION_POS+210;
                break;

        }
        Command specialLeftMove = new LambdaCommand("specialLeftMove",()->true);
        specialLeftMove
                .then(moveBezier("leftMove",true,preloadDepoLeft,preloadLeftPush))
                .branch(controller.drop())
                .branch(controller.retractIntake())
                .then(moveBezier("parkMove", preloadLeftPush, parkPos));
        ...
        startCommand
                .then(new CompositeCommand("depositPreload",
                        ...
                        Command.box(moveBezier("initialMove",startPos, preloadDepoPos)
                                ...
                                .then(new ConditionalBranches("leftSpecialCondition", Arrays.asList(
        ...
        while (opModeIsActive() && !scheduler.activeCommands.isEmpty()) {
            super.tick();
        }
    }
}
